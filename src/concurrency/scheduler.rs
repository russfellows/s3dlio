...
impl Throughput {
    ...
    pub fn new_gbps(gbps: u64) -> Self {
        Self {
            bytes_per_sec: gbps * 1_000_000_000,
        }
    }
...
impl AdaptiveScheduler {
    ...
    fn calculate_optimal_concurrency(
        ...
        std::cmp::max(
            min_concurrency,
            std::cmp::min(max_concurrency, needed_connections as usize),
        )
    }
    ...
    pub fn adapt_concurrency(&self) {
        if let ConcurrencyMode::Auto | ConcurrencyMode::TargetThroughput(_) = &self.mode {
            ...
            if current_throughput.bytes_per_sec() > 0 && inflight > 0 {
                let throughput_per_connection =
                    current_throughput.bytes_per_sec() / inflight as u64;
                ...
                self.optimal_concurrency
                    .store(new_optimal, Ordering::Relaxed);
            }
        }
    }
...
impl<'a> SchedulerPermit<'a> {
    ...
    pub fn complete(self, bytes_transferred: u64) {
        ...
        self.scheduler
            .metrics
            .record_completion(bytes_transferred, duration);
        self.scheduler
            .metrics
            .inflight_operations
