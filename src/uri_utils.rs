...
pub fn expand_uri_template(template: &str) -> Result<Vec<String>> {
    ...
    for cap in range_re.captures_iter(template) {
        let start: i32 = cap[1].parse().context("Invalid range start number")?;
        let end: i32 = cap[2].parse().context("Invalid range end number")?;
    ...
    for uri in &results[1..] {
        ...
        if scheme != first_scheme {
            bail!(
                "URI expansion resulted in mixed schemes: {} and {}",
                first_scheme,
                scheme
            );
        }
...
pub fn parse_uri_list(uri_str: &str) -> Result<Vec<String>> {
    ...
    for uri in &uris[1..] {
        ...
        if scheme != first_scheme {
            bail!(
                "URI list contains mixed schemes: {} and {}",
                first_scheme,
                scheme
            );
        }
...
pub fn load_uris_from_file(path: &Path) -> Result<Vec<String>> {
    ...
    for uri in &uris[1..] {
        ...
        if scheme != first_scheme {
            bail!(
                "URI file contains mixed schemes: {} and {}",
                first_scheme,
                scheme
            );
        }
...
mod tests {
    ...
    fn test_expand_simple_range() {
        ...
        assert_eq!(
            result,
            vec!["s3://bucket-1/", "s3://bucket-2/", "s3://bucket-3/"]
        );
    }
    ...
    fn test_parse_comma_separated() {
        let result =
            parse_uri_list("s3://host1/bucket/, s3://host2/bucket/, s3://host3/bucket/").unwrap();
        assert_eq!(
            result,
            vec![
                "s3://host1/bucket/",
    ...
    fn test_parse_comma_separated_with_extra_spaces() {
        ...
        assert_eq!(result, vec!["s3://host1/", "s3://host2/"]);
    }
    ...
    fn test_infer_scheme() {
        ...
        assert_eq!(
            infer_scheme_from_uri("direct:///dev/nvme0n1").unwrap(),
            "direct"
        );
