...
use crate::constants::{
    DEFAULT_CONCURRENT_UPLOADS, DEFAULT_S3_MULTIPART_PART_SIZE, MIN_S3_MULTIPART_PART_SIZE,
};
...
impl AdaptiveParams {
        ...
        match file_size {
            ...
            Some(size) => {
                ...
                match workload {
                    ...
                    WorkloadType::SmallFile => (8 * 1024 * 1024)
                        .max(self.config.min_part_size)
                        .min(self.config.max_part_size),
                    // Medium files: Use default parts (16 MB) - proven performance
                    WorkloadType::MediumFile => DEFAULT_S3_MULTIPART_PART_SIZE
                        .max(self.config.min_part_size)
                        .min(self.config.max_part_size),
                    // Large files: Use larger parts (32 MB) for better throughput
                    WorkloadType::LargeFile => (32 * 1024 * 1024)
                        .max(self.config.min_part_size)
                        .min(self.config.max_part_size),
                    // Batch: Use medium parts (16 MB) for balanced performance
                    WorkloadType::Batch => DEFAULT_S3_MULTIPART_PART_SIZE
                        .max(self.config.min_part_size)
                        .min(self.config.max_part_size),
                    // Unknown: Use default
                    WorkloadType::Unknown => DEFAULT_S3_MULTIPART_PART_SIZE
                        .max(self.config.min_part_size)
                        .min(self.config.max_part_size),
                }
            }
    ...
    pub fn compute_concurrency(
        ...
        let workload = workload_type
            .or(self.config.workload_type)
        ...
        match workload {
            ...
            // Unknown: Use default
            WorkloadType::Unknown => DEFAULT_CONCURRENT_UPLOADS
                .max(self.config.min_concurrency)
                .min(self.config.max_concurrency),
        }
    }
...
mod tests {
    ...
    fn test_workload_type_from_file_size() {
        assert_eq!(
            WorkloadType::from_file_size(1024 * 1024),
            WorkloadType::SmallFile
        );
        assert_eq!(
            WorkloadType::from_file_size(100 * 1024 * 1024),
            WorkloadType::MediumFile
        );
        assert_eq!(
            WorkloadType::from_file_size(500 * 1024 * 1024),
            WorkloadType::LargeFile
        );
    }
    ...
    fn test_adaptive_respects_bounds() {
        let config =
            AdaptiveConfig::enabled().with_part_size_bounds(10 * 1024 * 1024, 20 * 1024 * 1024); // 10-20 MB
        let params = AdaptiveParams::new(config);
