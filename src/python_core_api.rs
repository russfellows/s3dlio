...
fn opts_from_dict(opts: Option<Bound<'_, PyDict>>) -> LoaderOptions {
...
pub fn list_objects(bucket: &str, prefix: &str, recursive: bool) -> PyResult<Vec<String>> {
...
#[pyfunction]
pub fn stat_many_async<'py>(py: Python<'py>, uris: Vec<String>) -> PyResult<pyo3::Bound<'py, PyAny>> {
    future_into_py(py, async move {
...
// ---------------------------------------------------------------------------
// Dataset classes needed for core operations
// ---------------------------------------------------------------------------

#[pyclass]
#[derive(Clone)]
pub struct PyS3Dataset { 
    pub inner: S3BytesDataset 
}
