...
pub async fn read_latest(store: &dyn ObjectStore, latest_uri: &str) -> Result<Option<Latest>> {
...
pub async fn delete_latest(store: &dyn ObjectStore, latest_uri: &str) -> Result<()> {
    store.delete(latest_uri).await
}

/// Check if a latest pointer exists
pub async fn latest_exists(store: &dyn ObjectStore, latest_uri: &str) -> Result<bool> {
...
mod tests {
    ...
    async fn test_latest_operations() -> Result<()> {
        ...
        // Write first latest
        let latest1 = Latest::new(
            "manifest1.json".to_string(),
            100,
        ...
        let latest2 = Latest::new(
            "manifest2.json".to_string(),
            200,
            "2024-01-01T13:00:00Z".to_string(),
        );
        ...
        let latest3 = Latest::new(
            "manifest3.json".to_string(),
            150,
            "2024-01-01T14:00:00Z".to_string(),
        );
    ...
    async fn test_timestamp_comparison() -> Result<()> {
        ...
        let latest3 = Latest::new(
            "manifest3.json".to_string(),
            100,
            "2024-01-01T11:00:00Z".to_string(),
        );
        assert!(!update_latest_safe(&*store, &latest_uri, &latest3).await?);
    ...
    async fn test_atomic_latest_pointer_updates() -> Result<()> {
        ...
        if let Some((step, ts)) =
            parse_latest_marker_key("latest/ckpt-100-2024-01-01T13:00:00Z.json")
        {
            assert_eq!(step, 100);
        ...
        let latest_info = Latest::new(
            "manifest-42.json".to_string(),
            42,
            "2024-01-01T12:00:00Z".to_string(),
        );
        ...
        let loaded_info = read_latest(&store as &dyn ObjectStore, &latest_uri)
            .await?
            .unwrap();
        ...
        store
            .put(
                &src_uri,
                bytes::Bytes::from_static(b"test atomic rename content"),
            )
            .await?;
    ...
    async fn test_atomic_rename_cross_directory() -> Result<()> {
        ...
        store
            .put(
                &src_uri,
                bytes::Bytes::from_static(b"cross-directory rename test"),
            )
            .await?;
        assert!(store.exists(&src_uri).await?);
