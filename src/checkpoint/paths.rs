...
impl std::str::FromStr for Strategy {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => anyhow::bail!(
                "Unknown strategy: {}. Valid options: flat, round_robin, binary",
                s
            ),
        }
    }
...
impl KeyLayout {
    ...
    pub fn shard_dir_prefix(&self, strat: Strategy) -> String {
        match strat {
            ...
            Strategy::RoundRobin => "shards/rr/".to_string(),
            Strategy::Binary => "shards/bin/".to_string(),
        }
    }
...
mod tests {
    ...
    fn test_shard_keys() {
        let layout =
            KeyLayout::with_timestamp("s3://bucket/ckpt".to_string(), 50, "test".to_string());
    ...
    fn test_parse_manifest_key() {
        let result =
            KeyLayout::parse_manifest_key("s3://bucket/manifests/ckpt-100-20240101-120000.json");
        assert!(result.is_ok());
