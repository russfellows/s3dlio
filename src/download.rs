...
use crate::memory::{AlignedBuf, BufferPool};
...
impl DownloadEngine {
    ...
    pub async fn download_object(&self, uri: &str, target: Target) -> Result<DownloadStats> {
        ...
        match target {
            Target::Memory { pool } => self.download_to_memory(uri, pool).await,
    ...
    pub async fn download_objects<F>(
        &self,
        uris: &[String],
        mut target_fn: F,
        ...
        F: FnMut(&str) -> Target,
    {
        ...
        total_stats.throughput_bps =
            total_stats.total_bytes as f64 / total_stats.elapsed.as_secs_f64();
    ...
    pub async fn stream_download_parts(
        ...
        anyhow::bail!(
            "Stream download not yet implemented for URI: {}, pool capacity: {}",
            uri,
            pool.capacity()
        )
    }
    ...
    async fn download_to_memory(&self, uri: &str, pool: Arc<BufferPool>) -> Result<DownloadStats> {
        ...
        anyhow::bail!(
            "Memory download not yet implemented. URI: {}, Pool capacity: {}",
            uri,
            pool.capacity()
        )
    }
    ...
    async fn download_to_file(
        ...
        anyhow::bail!(
            "File download not yet implemented. URI: {}, Path: {}, Preallocate: {}",
            uri,
            path.display(),
            preallocate
        )
