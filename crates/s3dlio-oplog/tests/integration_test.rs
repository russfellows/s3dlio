...
use s3dlio_oplog::{replay_workload, translate_uri, OpExecutor, OpLogReader, OpType, ReplayConfig};
...
impl OpExecutor for TestExecutor {
    ...
    async fn put(&self, uri: &str, bytes: usize) -> Result<()> {
        self.operations
            .lock()
            .unwrap()
            .push(format!("PUT {} ({} bytes)", uri, bytes));
        Ok(())
    }
...
async fn test_continue_on_error() -> Result<()> {
    ...
    assert_eq!(
        ops.len(),
        2,
        "Should have executed 2 GET operations despite PUT failure"
    );
...
async fn test_s3_bench_13_column_tsv() -> Result<()> {
    ...
    assert_eq!(
        entries.len(),
        3,
        "Should parse all 3 operations from 13-column format"
    );
    assert_eq!(entries[0].op, OpType::GET);
...
async fn test_head_to_stat_alias() -> Result<()> {
    ...
    writeln!(
        file,
        "0\tHEAD\t0\thttp://s3.local\t/bucket/object.bin\t1000000000\t5000000\t"
    )?;
    file.flush()?;
    ...
    assert_eq!(ops.len(), 1);
    assert!(
        ops[0].starts_with("STAT"),
        "HEAD should map to STAT operation"
    );
...
async fn test_speed_multiplier_timing() -> Result<()> {
    ...
    writeln!(
        file,
        "0\tGET\t1024\thttp://s3\t/file1\t2025-01-01T00:00:00Z\t5000000\t"
    )?;
    writeln!(
        file,
        "1\tGET\t1024\thttp://s3\t/file2\t2025-01-01T00:00:01Z\t5000000\t"
    )?;
    file.flush()?;
    ...
    let gap_ns = entries[1].start.timestamp_nanos_opt().unwrap()
        - entries[0].start.timestamp_nanos_opt().unwrap();
    let gap_ms = gap_ns / 1_000_000;
    let adjusted_delay_ms = (gap_ms as f64 / speed) as u64;

    // 1000ms gap / 10x speed = 100ms delay
    assert_eq!(
        adjusted_delay_ms, 100,
        "Speed multiplier should reduce delay by 10x"
    );
...
async fn test_extra_columns_ignored() -> Result<()> {
    ...
    writeln!(
        file,
        "0\tGET\t1024\t/data/file.bin\textra_value\t42\t1000000000\t"
    )?;
    file.flush()?;
...
async fn test_minimal_vs_extended_formats() -> Result<()> {
    ...
    writeln!(
        minimal_file,
        "GET\t/data/small.bin\t512\t1000000000\t1000000\t"
    )?;
    minimal_file.flush()?;
...
async fn test_zstd_auto_detection() -> Result<()> {
    ...
    writeln!(
        encoder,
        "GET\t/compressed/file.bin\t2048\t1000000000\t5000000\t"
    )?;
    encoder.finish()?;
...
async fn test_dl_driver_jsonl_format() -> Result<()> {
    ...
    writeln!(
        file,
        r#"{{"operation": "GET", "file": "/test/file1.dat", "bytes": 1024, "t_start_ns": 1000000000}}"#
    )?;
    writeln!(
        file,
        r#"{{"operation": "PUT", "file": "/test/file2.dat", "bytes": 2048, "t_start_ns": 1500000000}}"#
    )?;
    file.flush()?;
