...
async fn test_allocation_overhead_comparison() -> Result<()> {
    ...
    store_no_pool
        .put(&uri_no_pool, test_data.clone().into())
        .await?;
    ...
    println!(
        "   Average per read: {:.2}µs",
        time_no_pool_1.as_micros() as f64 / 1000.0
    );
    ...
    store_with_pool
        .put(&uri_with_pool, test_data.clone().into())
        .await?;
    ...
    println!(
        "   Average per read: {:.2}µs",
        time_with_pool_1.as_micros() as f64 / 1000.0
    );
    ...
    let uri_with_pool_2 = format!(
        "file://{}/test_with_pool_2.dat",
        base_path.to_str().unwrap()
    );

    store_with_pool_2
        .put(&uri_with_pool_2, test_data.clone().into())
        .await?;
    let _ = benchmark_range_reads(&store_with_pool_2, &uri_with_pool_2, 10).await?;
    let time_with_pool_2 =
        benchmark_range_reads(&store_with_pool_2, &uri_with_pool_2, 1000).await?;
    println!("   Time for 1000 range reads: {:?}", time_with_pool_2);
    println!(
        "   Average per read: {:.2}µs",
        time_with_pool_2.as_micros() as f64 / 1000.0
    );
    ...
    store_no_pool_2
        .put(&uri_no_pool_2, test_data.clone().into())
        .await?;
    ...
    println!(
        "   Average per read: {:.2}µs",
        time_no_pool_2.as_micros() as f64 / 1000.0
    );
    ...
    println!(
        "   Test order impact: {:.1}% difference between orderings",
        (diff_1 - diff_2).abs()
    );
...
async fn test_memory_allocation_stats() -> Result<()> {
    ...
    let config_no_pool = FileSystemConfig::default();
    let store_no_pool = ConfigurableFileSystemObjectStore::new(config_no_pool);
    let uri_no_pool = format!(
        "file://{}/test_mem_no_pool.dat",
        base_path.to_str().unwrap()
    );
    store_no_pool
        .put(&uri_no_pool, test_data.clone().into())
    ...
    for i in 0..100 {
        ...
        let _data = store_no_pool
            .get_range(&uri_no_pool, offset, Some(1024 * 1024))
            .await?;
    }
    ...
    let config_with_pool = FileSystemConfig::direct_io();
    let store_with_pool = ConfigurableFileSystemObjectStore::new(config_with_pool);
    let uri_with_pool = format!(
        "file://{}/test_mem_with_pool.dat",
        base_path.to_str().unwrap()
    );
    store_with_pool
        .put(&uri_with_pool, test_data.clone().into())
    ...
    for i in 0..100 {
        let offset = (i * 1024 * 1024) % (64 * 1024 * 1024 - 1024 * 1024);
        let _data = store_with_pool
            .get_range(&uri_with_pool, offset, Some(1024 * 1024))
            .await?;
    }
    ...
    println!(
        "   RSS increase: {} KB",
        rss_after_pool.saturating_sub(rss_before_pool)
    );
