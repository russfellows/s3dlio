...
use s3dlio::data_gen_alt::{default_data_gen_threads, total_cpus, GeneratorConfig};
...
fn test_cpu_utilization() {
    ...
    println!(
        "  Default threads: {} ({}%)",
        default_threads,
        (default_threads * 100) / total_cores
    );
    ...
    println!(
        "  Generating {} GB of data in {} MB chunks (streaming API like dgen-rs)...",
    ...
    println!(
        "\n  Expected minimum: {:.2} GB/s (6 physical cores × 8 GB/s)",
        expected_min_total_throughput
    );
    ...
    println!(
        "  All {} CPUs appear to be working optimally\n",
        total_cores
    );
}
