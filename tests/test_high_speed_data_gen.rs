...
use s3dlio::data_gen_alt::{generate_data, generate_data_simple, DataGenerator, GeneratorConfig};
...
fn test_data_gen_sanity_check() {
    if cfg!(debug_assertions) {
        ...
        println!(
            "Skipping performance threshold in non-optimized build (debug assertions enabled)"
        );
        return;
    }
    ...
    assert!(
        throughput_gbps > 1.0,
        "Throughput too low: {:.2} GB/s (expected > 1 GB/s)",
        throughput_gbps
    );
...
fn test_dedup_correctness() {
    ...
    assert!(
        (dedup_ratio - expected_ratio).abs() < 0.10,
        "Dedup ratio {:.4} not within 10% of expected {:.4}",
        dedup_ratio,
        expected_ratio
    );
...
fn test_compression_correctness() {
    ...
    // Allow 5% tolerance
    assert!(
        (zero_ratio - expected_ratio).abs() < 0.05,
        "Compression ratio {:.4} not within 5% of expected {:.4}",
        zero_ratio,
        expected_ratio
    );
...
fn test_high_speed_throughput() {
    ...
    println!(
        "Target: {} GB/s (based on dgen-rs performance)",
        MINIMUM_EXPECTED_THROUGHPUT_GBPS
    );
    ...
    if throughput_gbps >= adjusted_threshold {
        println!(
            "✅ PASSED: {:.2} GB/s >= {:.2} GB/s threshold",
            throughput_gbps, adjusted_threshold
        );
    } else {
        println!(
            "⚠️  Below threshold: {:.2} GB/s < {:.2} GB/s",
            throughput_gbps, adjusted_threshold
        );
        #[cfg(not(debug_assertions))]
        panic!(
...
fn test_streaming_throughput_optimal_chunks() {
    ...
    assert!(
        throughput_gbps > 5.0,
        "Streaming throughput too low: {:.2} GB/s",
        throughput_gbps
    );
