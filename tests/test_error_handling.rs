...
fn test_zero_size_handling() {
    ...
    println!(
        "Zero size streaming result: {} bytes in {} chunks",
...
fn test_extreme_parameters() {
    ...
    for (name, size, dedup, compress) in test_cases {
        println!(
            "Testing {}: size={}, dedup={}, compress={}",
            name, size, dedup, compress
        );
        ...
        if generation_time.as_secs() < 10 {
            ...
            println!(
                "  ⚠️ {} took longer than expected: {:.2}s",
                name,
...
fn test_memory_pressure() {
    ...
    println!(
        "Testing {} concurrent {} MB generations",
        num_concurrent,
    ...
    println!(
        ...
        (num_concurrent * 256) / 1024
    ); // Conservative chunk-based estimate

    assert!(
        total_generated > 0,
        "No data generated under memory pressure"
    );
    assert!(
        max_duration.as_secs() < 30,
        "Generation took too long under memory pressure"
    );
...
fn test_rapid_generator_creation() {
    ...
    println!(
        "Creating {} generators rapidly with {} KB each",
        num_generators,
        small_size / 1024
    );
    ...
    assert!(
        creation_rate > 100.0,
        "Generator creation rate too slow: {:.1}/s",
        creation_rate
    );
    ...
    for &chunk_size in &chunk_sizes {
        ...
        while let Some(chunk) = obj_gen.fill_chunk(chunk_size) {
            ...
            if chunk_count > 100000 {
                println!(
                    "⚠️ Stopping after {} chunks for chunk size {} (likely edge case)",
        ...
        println!(
            "  Generated {} bytes in {} chunks ({:.2} MB/s)",
        ...
        assert!(
            total_bytes > 0,
            "No data generated for chunk size {}",
            chunk_size
        );
        assert!(
            chunk_count > 0,
            "No chunks generated for chunk size {}",
            chunk_size
        );

        // For reasonable chunk sizes, verify efficiency
        if (1024..=1024 * 1024).contains(&chunk_size) {
            assert!(
                throughput_mb_s > 100.0,
                "Low throughput for chunk size {}: {:.2} MB/s",
                chunk_size,
                throughput_mb_s
            );
        }
...
fn test_streaming_consistency_across_restarts() {
    ...
    assert_eq!(
        continuous_data, restarted_data,
        "Data mismatch between continuous and restarted generation"
    );

    println!(
        "Generated {} bytes continuously and restarted - identical results",
        continuous_data.len()
    );
    ...
    assert_ne!(
        continuous_data, different_data,
...
fn test_reproducible_generation_with_seed() {
    ...
    assert_eq!(
        data1, data2,
        "Data mismatch for same seed - not reproducible!"
    );

    println!(
        "Second generator with same seed produced identical {} bytes",
        data2.len()
    );
    ...
    assert_ne!(
        data1, data3,
        "Different seeds produced identical data (unexpected)"
    );

    println!(
        "Different seed {:#018X} produced different data as expected",
        different_seed
    );
