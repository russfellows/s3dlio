...
async fn test_checkpoint_writer_with_checksums() -> Result<()> {
    ...
    let shard_meta = writer
        .finalize_writer_to_shard_meta(&layout, key, shard_writer)
        .await?;
...
async fn test_checkpoint_manifest_with_checksums() -> Result<()> {
    ...
    for i in 0..3 {
        ...
        let test_data = format!(
            "Shard {} data with unique content for checksum verification",
            i
        );
    ...
    let checksums: Vec<_> = shard_metas
        .iter()
        .map(|s| s.checksum.as_ref().unwrap())
        .collect();
    for i in 0..checksums.len() {
        for j in i + 1..checksums.len() {
            assert_ne!(
                checksums[i], checksums[j],
                "Checksums should be unique for different data"
            );
        }
    }
    ...
    println!(
        "✓ Created manifest with {} shards, all containing checksums",
        manifest.shards.len()
    );
...
async fn test_checkpoint_integrity_validation() -> Result<()> {
    ...
    let layout = s3dlio::checkpoint::KeyLayout::new("integrity_test".to_string(), 200);

    // Write data and capture checksum
    let (mut shard_writer, key) = writer.get_shard_writer(&layout).await?;
    let original_data =
        b"Critical data that must maintain integrity across storage and retrieval operations.";
    shard_writer.write_chunk(original_data).await?;

    let expected_checksum = shard_writer.checksum().unwrap();
    let shard_meta = writer
        .finalize_writer_to_shard_meta(&layout, key.clone(), shard_writer)
        .await?;
    ...
    println!(
        "✓ Data integrity verified: original checksum {} matches recomputed checksum",
        expected_checksum
    );
