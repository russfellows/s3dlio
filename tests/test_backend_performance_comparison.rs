...
impl PerformanceMetrics {
    fn print_summary(&self) {
        println!(
            "📊 {} {} Performance ({}MB objects):",
            self.backend_name, self.operation, self.object_size_mb
        );
...
async fn benchmark_put_operations(
    ...
    put_objects_with_random_data_and_type(&object_keys, object_size, concurrency, config)?;
    ...
    let avg_latency_ms =
        (total_duration.as_millis() as f64) / (num_operations as f64 / concurrency as f64);
...
async fn benchmark_get_operations(
    ...
    let avg_latency_ms =
        (total_duration.as_millis() as f64) / (results.len() as f64 / concurrency as f64);
...
async fn run_performance_test_for_size(
    object_size: usize,
    ...
    println!("\n{}", "=".repeat(60));
    println!(
        "🎯 Performance Test: {}MB Objects with {} Backend",
        object_size as f64 / (1024.0 * 1024.0),
        get_backend_name()
    );
    println!("{}", "=".repeat(60));
    ...
    let get_metrics = benchmark_get_operations(&object_keys, GET_CONCURRENCY).await?;
...
fn print_environment_info() {
    ...
    println!(
        "   Test Duration: {}s per operation type",
        MIN_TEST_DURATION.as_secs()
    );
}
...
fn print_summary_table(all_results: &[(PerformanceMetrics, PerformanceMetrics)]) {
    ...
    // Calculate totals
    let total_put_ops: usize = all_results
        .iter()
        .map(|(p, _)| p.operations_completed)
        .sum();
    let total_get_ops: usize = all_results
        .iter()
        .map(|(_, g)| g.operations_completed)
        .sum();
