...
use s3dlio::object_store::{CompressionConfig, ObjectStore, WriterOptions};
...
async fn test_direct_io_streaming_writer() -> Result<()> {
    ...
    if !ConfigurableFileSystemObjectStore::test_direct_io_support(temp_dir.path()).await {
        println!(
            "⚠️  Skipping DirectIO streaming test - O_DIRECT not supported on this filesystem"
        );
        return Ok(());
    }
...
async fn test_large_streaming_write() -> Result<()> {
    ...
    println!(
        "✅ Large streaming write test passed ({} MB)",
