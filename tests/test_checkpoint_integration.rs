...
async fn test_basic_checkpoint_operations() {
    ...
    let manifest_key = writer
        .save_checkpoint(1, 1, "pytorch", test_data, Some(user_meta_json))
        .await
        .unwrap();
    ...
    assert!(
        latest_manifest_opt.is_some(),
        "Should have found a checkpoint"
    );
    ...
    assert_eq!(
        latest_manifest.user_meta,
        Some(serde_json::to_value(user_metadata).unwrap())
    );
    ...
    let loaded_data = reader
        .read_shard_by_rank(&latest_manifest, 0)
        .await
        .unwrap();
...
async fn test_checkpoint_validation() {
    ...
    let _manifest_key = writer
        .save_checkpoint(1, 1, "pytorch", b"test data", None)
        .await
        .unwrap();
...
async fn test_checkpoint_versioning() {
    ...
    let epochs = [1, 3, 2, 5]; // Out of order to test sorting
    ...
    println!(
        "✓ Latest checkpoint correctly identified as epoch {}",
        latest.epoch
    );
}
