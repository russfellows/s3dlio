...
fn test_multi_npz_two_arrays() -> Result<()> {
    ...
    let arrays = vec![("images", &images), ("labels", &labels)];
...
fn test_multi_npz_pytorch_style() -> Result<()> {
    ...
    // Labels: [batch_size]
    let labels = ArrayD::from_shape_vec(vec![batch_size], vec![3.0, 7.0, 2.0, 9.0])?;

    // Metadata: single value (epoch number or similar)
    let epoch = ArrayD::from_shape_vec(vec![1], vec![5.0])?;

    let arrays = vec![("images", &images), ("labels", &labels), ("epoch", &epoch)];
