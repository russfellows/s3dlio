...
use s3dlio::data_loader::{
    AsyncPoolDataLoader, DataLoader, LoaderOptions, MultiBackendDataset, PoolConfig,
};
...
async fn test_dataloader_cancellation_during_streaming() {
    ...
    match remaining_batches {
        Ok(count) => {
            println!(
                "✅ Received {} additional batches after cancellation (draining)",
                count
            );
            assert!(
                count < 50,
                "Should not receive too many batches after cancellation"
            );
        }
        Err(_) => {
...
async fn test_dataloader_cancellation_with_delay() {
    ...
    println!(
        "✅ Received {} total batches with delayed cancellation",
        total_batches
    );
    assert!(total_batches > 0, "Should receive at least some batches");
    assert!(
        total_batches < 50,
        "Should not receive all 50 batches (500 items / 10 batch_size)"
    );
}
...
async fn test_async_pool_dataloader_cancellation_during_streaming() {
    ...
    match remaining_batches {
        Ok(count) => {
            println!(
                "✅ AsyncPool received {} additional batches after cancellation",
                count
            );
            assert!(
                count < 50,
...
async fn test_async_pool_dataloader_cancellation_stops_new_requests() {
    ...
    println!(
        "✅ AsyncPool drained {} batches after cancellation (pool_size=4)",
        remaining_count
    );
    assert!(
        remaining_count <= 10,
        "Should drain approximately pool_size worth of batches"
    );
}
...
async fn test_cancellation_without_token() {
    ...
    match result {
        Ok(count) => {
            println!(
                "✅ Without cancellation token: received {} batches, stream completed",
                count
            );
