...
fn test_single_process_baseline() {
    ...
    println!(
        "Testing {} iterations of {} MB buffers ({} MB total)",
        iterations,
    ...
    for i in 0..iterations {
        ...
        if i % 10 == 0 {
            ...
            println!(
                "Progress: {} MB generated, {:.2} GB/s current",
    ...
    println!(
        "✅ Single process baseline established: {:.2} GB/s",
        throughput
    );
}
...
fn test_multi_process_throughput() {
    ...
    for &num_processes in &process_counts {
        ...
        println!(
            "Configuration: {} processes × {} iterations × {} MB = {} MB total",
        ...
        match create_test_worker() {
            Ok(_worker_path) => {
                ...
                let estimated_multi_process_time =
                    estimated_single_process_time / (num_processes as f64 * 0.8); // 80% efficiency
                let estimated_throughput =
                    (total_mb as f64) / (1024.0 * estimated_multi_process_time);
                ...
                println!(
                    "Estimated aggregate throughput: {:.2} GB/s",
                    estimated_throughput
                );

                if estimated_throughput >= 20.0 {
                    println!(
                        "✅ Target achieved: {:.2} GB/s >= 20 GB/s",
                        estimated_throughput
                    );
                } else {
                    println!(
                        "⚠️ Target not met: {:.2} GB/s < 20 GB/s",
                        estimated_throughput
                    );
                }
            }
...
fn test_multi_process_data_uniqueness() {
    ...
    println!(
        "Simulating {} processes, {} iterations each",
    ...
    for process_id in 0..num_simulated_processes {
        for iteration in 0..iterations_per_process {
            ...
            if all_hashes.contains(&hash) {
                collision_count += 1;
                println!(
                    "⚠️ Hash collision detected for process {}, iteration {}",
            ...
            if iteration == 0 {
                println!(
                    "Process {}: Generated {} bytes, hash: {:016x}",
                    process_id,
                    data.len(),
                    hash
                );
    ...
    assert!(
        unique_ratio >= 0.95,
        "Uniqueness ratio too low: {:.3}",
        unique_ratio
    );

    println!(
        "✅ Multi-process data uniqueness verified: {:.1}% unique",
        unique_ratio * 100.0
    );
}
...
fn test_scaling_analysis() {
    ...
    println!(
        "Processes needed (100% efficiency): {:.1}",
        ideal_processes_needed
    );
    println!(
        "Processes needed (80% efficiency): {:.1}",
        conservative_processes_needed
    );
