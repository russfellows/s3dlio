...
async fn test_direct_io_basic_operations() -> Result<()> {
    ...
    let supports_direct_io =
        ConfigurableFileSystemObjectStore::test_direct_io_support(base_path).await;
...
async fn test_high_performance_config() -> Result<()> {
    ...
    store
        .put_multipart(&test_uri, test_data.clone(), Some(64 * 1024))
        .await?;
...
async fn test_large_file_with_direct_io() -> Result<()> {
    ...
    let chunk3 = store
        .get_range(&test_uri, test_data.len() as u64 - 4096, None)
        .await?;
...
async fn test_factory_functions() -> Result<()> {
    ...
    hp_store
        .put(&test_uri, bytes::Bytes::from(large_data.clone()))
        .await?;
