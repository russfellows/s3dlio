...
async fn test_range_engine_decision_with_cached_size() -> Result<()> {
    ...
    for (filename, size_mb) in test_cases {
        ...
        if size_mb >= 32 {
            println!(
                "  → RangeEngine should be beneficial for {}MB files",
                size_mb
            );
        } else {
...
async fn test_range_engine_throughput_comparison() -> Result<()> {
    ...
    let estimated_throughput =
        (data_simple.len() as f64 / 1_048_576.0) / estimated_range_engine_time;

    println!("  Expected with RangeEngine on S3/GCS:");
    println!(
        "    Time: ~{:.2}s (vs {:.2}s)",
        estimated_range_engine_time,
        simple_time.as_secs_f64()
    );
    println!(
        "    Throughput: ~{:.2} MB/s (vs {:.2} MB/s)",
        estimated_throughput, simple_throughput
    );
    ...
    println!(
        "  ✓ For 64MB files, RangeEngine expected to provide 30-50% speedup on network storage"
    );
...
async fn test_cache_enables_optimal_strategy_selection() -> Result<()> {
    ...
    println!(
        "  ✓ Total stat count: {} (pre-stat only, not {} per file)",
        files.len(),
        files.len()
    );
...
async fn test_cache_miss_vs_cache_hit_performance() -> Result<()> {
    ...
    if miss_time > hit_time {
        ...
        println!(
            "  Difference: {:?} ({:.1}% faster with cache)",
            diff, percent_faster
        );

        if percent_faster < 5.0 {
            println!(
                "  → Stat overhead minimal for local filesystem (~{:?})",
                diff
            );
