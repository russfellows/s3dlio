...
use s3dlio::{FileSystemObjectStore, ObjectStore};
use std::fs;
use std::sync::Arc;
...
async fn test_pre_stat_performance_improvement() -> Result<()> {
    ...
    println!(
        "  Downloaded {} files in {:?}",
        file_count, without_prestat_time
    );
    println!(
        "  Total bytes: {} ({:.2} MB)",
        total_bytes,
    ...
    println!(
        "  Average per file: {:?}",
        without_prestat_time / file_count as u32
    );
    ...
    println!(
        "  Pre-stat phase: {} files in {:?}",
        cached_count, prestat_time
    );
    ...
    println!(
        "  Download phase: {} files in {:?}",
        file_count, download_time
    );
    println!(
        "  Total time (pre-stat + download): {:?}",
        total_with_prestat
    );
    println!(
        "  Total bytes: {} ({:.2} MB)",
        total_bytes_cached,
        total_bytes_cached as f64 / 1_048_576.0
    );
    ...
    if total_with_prestat < without_prestat_time {
        ...
        println!(
            "  ✗ Note: For local filesystem with fast stat, cache overhead may exceed benefit"
        );
    }
...
async fn test_cache_expiration_and_refresh() -> Result<()> {
    ...
    if expired_time > immediate_time {
        ...
        println!(
            "  ✓ Cache expiration working: {:.2}x slower after expiration",
            ratio
        );
    } else {
...
async fn test_concurrent_access_performance() -> Result<()> {
    ...
    for uri in uris {
        ...
        let handle = tokio::spawn(async move { store_clone.get(&uri).await });
        handles.push(handle);
    }
...
async fn test_stat_operation_counting() -> Result<()> {
    ...
    println!(
        "  Expected stat count: {} (concurrent pre-stat only)",
        file_count
    );
    ...
    println!(
        "  With cache: {} concurrent stats + 0 during download",
        file_count
    );
    println!("  \n  For network storage (S3/GCS with 20ms latency per stat):");
    println!(
        "    Without: {} × 20ms = {:.1}s (sequential)",
        file_count,
        file_count as f64 * 0.02
    );
    println!(
        "    With: {} / 50 concurrent × 20ms = {:.1}s (concurrent pre-stat)",
        file_count,
        (file_count as f64 / 50.0) * 0.02
    );
...
async fn test_real_world_benchmark_simulation() -> Result<()> {
    ...
    println!(
        "Simulating sai3-bench workload: {} files × {} MB",
        file_count,
        file_size / 1_048_576
    );
    ...
    let traditional_throughput =
        (total_bytes as f64 / 1_048_576.0) / traditional_time.as_secs_f64();
    ...
    let optimized_throughput =
        (total_bytes_opt as f64 / 1_048_576.0) / total_optimized_time.as_secs_f64();
    ...
    println!(
        "  Traditional: {:?} @ {:.2} MB/s",
        traditional_time, traditional_throughput
    );
    println!(
        "  Optimized:   {:?} @ {:.2} MB/s",
        total_optimized_time, optimized_throughput
    );
