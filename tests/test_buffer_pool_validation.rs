...
fn create_aligned_file(path: &std::path::Path, size_mb: usize) -> Result<String> {
    ...
    for (i, byte) in data.iter_mut().enumerate().take(size_bytes) {
        *byte = (i % 256) as u8;
    }
...
async fn test_buffer_pool_vs_allocate_per_read() -> Result<()> {
    ...
    println!(
        "  Downloaded {:.2} MB in {:?}",
        total_bytes_file as f64 / 1_048_576.0,
        file_time
    );
    ...
    println!(
        "  DirectIO (with pool): {:?} @ {:.2} MB/s",
        pooled_time, pooled_throughput
    );
    println!(
        "  Standard I/O:         {:?} @ {:.2} MB/s",
        file_time, file_throughput
    );
...
async fn test_buffer_pool_memory_reuse() -> Result<()> {
    ...
    println!(
        "  ✓ Expected: ~15-20% faster than allocating {} × {}MB",
        file_count, file_size_mb
    );
...
async fn test_buffer_pool_claims_validation() -> Result<()> {
    ...
    println!(
        "  With buffer pool (actual):    {:.2} MB/s in {:?}",
        throughput, elapsed
    );
    println!(
        "  Without pool (estimated):     {:.2} MB/s in {:.2}s",
