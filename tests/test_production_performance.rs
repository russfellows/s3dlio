...
fn test_production_streaming_performance() {
    ...
    for &size in &sizes {
        println!(
            "\n--- Testing {} MB buffers (10 iterations) ---",
            size / (1024 * 1024)
        );
        ...
        println!(
            "Single-pass: {:.1} MB/s ({:.2}ms total for {} MB)",
            throughput,
            duration_ms,
        ...
        let throughput = throughput_mb_per_sec(total_bytes, duration_ms);
        println!(
            "Streaming:   {:.1} MB/s ({:.2}ms total for {} MB)",
            throughput,
            duration_ms,
            total_bytes / (1024 * 1024)
        );
...
fn test_sustained_generation_performance() {
    ...
    println!(
        "Generating {} buffers of {} MB each (1 GB total)",
        iterations,
...
fn test_rng_overhead_measurement() {
    ...
    println!(
        "Testing {} iterations of {} MB buffers",
        iterations,
        size / (1024 * 1024)
    );
    ...
    println!(
        "Per buffer: {:.2}ms for {:.1} MB = {:.1} MB/s per buffer",
        time_per_buffer,
        mb_per_buffer,
        mb_per_buffer / (time_per_buffer / 1000.0)
    );

    // Estimate RNG overhead
    let block_size = 65536; // BLK_SIZE
    let blocks_per_buffer = size.div_ceil(block_size);
    let unique_blocks = blocks_per_buffer / 4; // dedup_factor = 4
    println!(
        "Blocks per buffer: {}, Unique blocks: {}",
        blocks_per_buffer, unique_blocks
    );
    println!(
        "RNG creations per buffer: {} (current) vs {} (optimized)",
        blocks_per_buffer, unique_blocks
    );
}
