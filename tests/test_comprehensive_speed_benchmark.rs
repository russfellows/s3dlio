    ...
    for (size_name, size) in &test_cases {
        for (param_name, dedup, compress) in &dedup_compress_cases {
            let (single_pass_gbps, streaming_gbps) =
                benchmark_approaches(*size, *dedup, *compress).await?;
...
async fn benchmark_approaches(size: usize, dedup: usize, compress: usize) -> Result<(f64, f64)> {
    ...
    let avg_single_pass = single_pass_times
        .iter()
        .map(|t| t.as_secs_f64())
        .sum::<f64>()
        / ITERATIONS as f64;
    let avg_streaming =
        streaming_times.iter().map(|t| t.as_secs_f64()).sum::<f64>() / ITERATIONS as f64;
...
async fn test_end_to_end_upload_speed_comparison() -> Result<()> {
    ...
    println!("|----------|-----------|-------------------|--------|");
    println!(
        "| Single-Pass (CLI/Python) | {:.1} | {:.2} | {} |",
        single_pass_time.as_millis(),
        single_pass_gbps,
    ...
    println!(
        "\n**Result**: {} is {:.1}x faster for end-to-end uploads",
        winner, ratio
    );
...
async fn test_memory_pressure_vs_speed_tradeoff() -> Result<()> {
    ...
    for (size_name, size) in test_sizes {
        ...
        println!(
            "| {} | {:.2} | {:.1} | {:.2} | {:.1} | {:.1}x slower | {:.0}x less |",
            size_name,
            single_pass_gbps,
            single_pass_memory_mb,
            streaming_gbps,
            streaming_memory_mb,
            speed_penalty,
            memory_savings
    ...
    println!(
        "**Recommendation**: Use single-pass by default, streaming only when memory-constrained"
    );
