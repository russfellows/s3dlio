...
use s3dlio::object_store::{ObjectProperties, ObjectStore};
...
async fn test_rmdir_fails_on_non_empty_directory_without_recursive() -> Result<()> {
    ...
    store
        .put(&file_uri, bytes::Bytes::from_static(b"test content"))
        .await?;
    ...
    assert!(
        result.is_err(),
        "rmdir should fail on non-empty directory without recursive=true"
    );
...
async fn test_rmdir_recursive_removes_non_empty_directory() -> Result<()> {
    ...
    store
        .put(&file1_uri, bytes::Bytes::from_static(b"content 1"))
        .await?;
    store
        .put(&file2_uri, bytes::Bytes::from_static(b"content 2"))
        .await?;
...
async fn test_rmdir_on_nonexistent_directory_is_idempotent() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "rmdir should be idempotent (succeed on non-existent directory)"
    );
...
async fn test_update_metadata_not_supported_for_file_backend() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "update_metadata should not be supported for file:// backend"
    );
...
async fn test_update_properties_http_properties_not_supported() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "HTTP properties should not be supported for file:// backend"
    );
    assert!(result.unwrap_err().to_string().contains("not supported"));
...
async fn test_update_properties_storage_class_ignored() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "storage_class should be accepted (as no-op) for file:// backend"
    );
...
async fn test_concurrent_mkdir_operations() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move { store_clone.mkdir(&dir_uri).await });
