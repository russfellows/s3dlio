...
async fn test_buffer_pool_initialization() -> Result<()> {
    ...
    for i in 0..10 {
        let buf = tokio::time::timeout(std::time::Duration::from_secs(1), pool.take()).await;
...
async fn test_range_read_with_buffer_pool() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        (
            512 * 1024,
            Some(256 * 1024),
            "256KB range crossing alignment",
        ),
    ];
    ...
    for (offset, length, description) in test_cases {
        ...
        println!(
            "✅ Range read validated: {} (offset={}, length={})",
...
async fn test_unaligned_range_reads() -> Result<()> {
    ...
    let unaligned_cases = vec![
        ...
        (
            32768 + 13,
            Some(1024 + 37),
            "Random unaligned offset and length",
        ),
    ];
...
async fn test_concurrent_range_reads() -> Result<()> {
    ...
    for i in 0..64 {
        ...
        let task =
            tokio::spawn(
                async move { store_clone.get_range(&uri_clone, offset, Some(8192)).await },
            );
...
async fn test_range_larger_than_pool_buffer() -> Result<()> {
    ...
    let large_range = store
        .get_range(&test_uri, 0, Some(96 * 1024 * 1024))
        .await?;
    ...
    assert_eq!(
        large_range[96 * 1024 * 1024 - 1],
        test_data[96 * 1024 * 1024 - 1]
    );
...
async fn test_data_integrity_multiple_reads() -> Result<()> {
    ...
    println!(
        "✅ Data integrity validated across {} chunks (64KB each)",
        chunk_count
    );
