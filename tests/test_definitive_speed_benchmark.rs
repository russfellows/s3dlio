...
async fn test_chunk_size_optimization() -> Result<()> {
    ...
    for (chunk_name, chunk_size) in &chunk_sizes {
        ...
        let vs_single_pass = if streaming_performance > single_pass_baseline {
            format!(
                "{:.1}x faster",
                streaming_performance / single_pass_baseline
            )
        } else {
            format!(
                "{:.1}x slower",
                single_pass_baseline / streaming_performance
            )
        };
    ...
    println!("\n**OPTIMIZATION RESULT**:");
    println!(
        "Best streaming configuration: {} chunks ({:.2} GB/s)",
        best_chunk, best_streaming
    );

    if best_streaming > single_pass_baseline {
        println!(
            "✅ **RECOMMENDATION**: Use streaming with {} chunks for best performance",
            best_chunk
        );
    } else {
        println!("✅ **RECOMMENDATION**: Use single-pass generation for best performance");
        println!(
            "   Streaming provides {:.0}x memory reduction at {:.1}x speed cost",
            (size as f64) / (256.0 * 1024.0),
            single_pass_baseline / best_streaming
        );
    }
