...
use s3dlio::gcs_client::{parse_gcs_uri, GcsClient};

/// Test bucket name from environment variable
fn get_test_bucket() -> Result<String> {
    env::var("GCS_TEST_BUCKET").map_err(|_| {
        anyhow::anyhow!(
...
async fn test_gcs_put_get_object() -> Result<()> {
    ...
    client
        .put_object(&bucket, &object_key, Bytes::from_static(test_data))
        .await?;
...
async fn test_gcs_get_object_range() -> Result<()> {
    ...
    let range_data = client
        .get_object_range(&bucket, &object_key, 10, Some(10))
        .await?;
    ...
    let tail_data = client
        .get_object_range(&bucket, &object_key, 30, None)
        .await?;
...
async fn test_gcs_list_objects() -> Result<()> {
    ...
    println!("\nTest 1: List with prefix (recursive)");
    let keys = client
        .list_objects(&bucket, Some(&test_prefix), true)
        .await?;
    ...
    let keys_nonrecursive = client
        .list_objects(&bucket, Some(&test_prefix), false)
        .await?;
...
async fn test_gcs_multipart_upload() -> Result<()> {
    ...
    let metadata = client.stat_object(&bucket, &object_key).await?;
    assert_eq!(
        metadata.size, data_size as u64,
        "Size mismatch after multipart upload"
    );
...
async fn test_parse_gcs_uri() -> Result<()> {
    ...
    let invalid_uris = vec!["s3://wrong-scheme/key", "http://not-gcs/key", "gs://", ""];
