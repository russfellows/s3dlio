...
fn test_edge_case_buffer_sizes() {
    ...
    for &size in &edge_sizes {
        ...
        assert!(
            !data1.is_empty(),
            "Single-pass generated empty data for size {}",
            size
        );
        ...
        assert!(
            total_streamed > 0,
            "Streaming generated no data for size {}",
            size
        );
        ...
        if data1.len() != streamed_data.len() {
            println!(
                "⚠️ Size mismatch for size {}: single-pass {} vs streaming {}",
                size,
                data1.len(),
                streamed_data.len()
            );
        ...
        println!(
            "✅ Size {} - Single-pass: {} bytes, Streaming: {} bytes",
            size,
            data1.len(),
            streamed_data.len()
        );
    ...
    for (size, dedup, compress) in test_cases {
        ...
        if compress > 1 && data.len() >= 1000 {
            ...
            if compress >= 2 {
                assert!(
                    zero_ratio > 0.1,
                    "Expected more zeros with compress={}, got ratio={}",
                    compress,
                    zero_ratio
                );
            }
        ...
        println!(
            "✅ Compression case passed - generated {} bytes",
            data.len()
        );
    }
}
...
fn test_deterministic_behavior() {
    ...
    for i in 1..iterations {
        ...
        assert_eq!(
            results[0], results[i],
            "Deterministic failure: data mismatch - iteration 0 != iteration {}",
            i
        );
    }

    println!(
        "✅ Deterministic behavior verified across {} iterations with same seed",
        iterations
    );
}
...
fn test_unique_streams_by_default() {
    ...
    println!(
        "✅ All {} streams are unique - default behavior is correct",
        num_streams
    );
}
...
fn test_concurrent_generation() {
    ...
    println!(
        "Testing {} threads, {} iterations each, {} MB per iteration",
        num_threads,
    ...
    for handle in handles {
        ...
        println!(
            "Thread {} completed: {} MB in {:.2}ms = {:.2} GB/s",
            thread_id,
            bytes / (1024 * 1024),
            duration,
            throughput
    ...
    for i in 0..thread_samples.len() {
        for j in (i + 1)..thread_samples.len() {
            assert_ne!(
                thread_samples[i].1, thread_samples[j].1,
    ...
    assert!(
        aggregate_throughput > 0.1,
        "Aggregate throughput too low: {:.2} GB/s",
        aggregate_throughput
    );

    println!(
        "✅ Concurrent generation successful with {:.2} GB/s aggregate",
        aggregate_throughput
    );
}
