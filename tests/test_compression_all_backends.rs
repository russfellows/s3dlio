...
use s3dlio::object_store::{store_for_uri, CompressionConfig, ObjectWriter};
...
async fn test_filesystem_compression() -> Result<()> {
    ...
    let mut writer =
        FileSystemWriter::new_with_compression(file_path.clone(), CompressionConfig::zstd_level(5))
            .await?;
    ...
    assert!(
        ratio < 1.0,
        "Data should be compressed (ratio: {:.2})",
        ratio
    );
    println!(
        "✅ FileSystem compression test passed (ratio: {:.2})",
        ratio
    );
    Ok(())
}
...
async fn test_directio_compression() -> Result<()> {
    ...
    println!(
        "✅ DirectIO compression test passed (ratio: {:.2})",
        compression_ratio
    );
    Ok(())
}
...
async fn test_s3_compression_via_store() -> Result<()> {
    let s3_uri = "s3://test-bucket/test_s3_compression.dat".to_string();
...
async fn test_compression_levels_across_backends() -> Result<()> {
    ...
    for level in [1, 3, 5, 10, 15, 22] {
        ...
        println!(
            "✅ Compression level {} test passed (ratio: {:.2})",
...
async fn test_compression_disabled_across_backends() -> Result<()> {
    ...
    let mut writer =
        FileSystemWriter::new_with_compression(file_path.clone(), CompressionConfig::None).await?;
...
async fn test_compression_with_integrity() -> Result<()> {
    ...
    let mut writer =
        FileSystemWriter::new_with_compression(file_path.clone(), CompressionConfig::zstd_level(7))
            .await?;
    ...
    assert!(
        checksum.unwrap().starts_with("crc32c:"),
        "Should be CRC32C checksum"
    );
