...
fn test_s3dlio_array_to_npy_bytes() -> Result<()> {
    ...
    println!(
        "First 128 bytes (hex): {:02x?}",
        &npy_bytes[..128.min(npy_bytes.len())]
    );
    ...
    assert_eq!(
        npy_bytes.as_ptr(),
        npy_bytes2.as_ptr(),
        "Bytes clone should be zero-copy"
    );
...
fn test_npz_round_trip() -> Result<()> {
    use s3dlio::data_formats::npz::{array_to_npy_bytes, list_npz_arrays, read_npz_array};
    use std::io::Write;
