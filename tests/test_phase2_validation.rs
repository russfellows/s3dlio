            ...
            match store.get_writer(&full_uri).await {
                Ok(mut writer) => {
                    ...
                    match writer.finalize().await {
                        ...
                        Err(e) => println!(
                            "⚠ DirectIO finalize failed (expected in some test environments): {}",
                            e
                        ),
                    }
                }
                Err(e) => println!(
                    "⚠ DirectIO writer creation failed (expected in some test environments): {}",
                    e
                ),
            }
        }
        Err(e) => println!(
            "⚠ DirectIO store creation failed (expected in some test environments): {}",
            e
        ),
    }
