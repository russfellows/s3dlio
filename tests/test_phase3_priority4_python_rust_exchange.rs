...
use s3dlio::checkpoint::{Manifest, Reader};
...
async fn test_enhanced_checkpoint_loading_with_validation() -> Result<()> {
    ...
    let checkpoint_data =
        bytes::Bytes::from_static(b"enhanced checkpoint state data for priority 4");
    ...
    let loaded_data = store
        .load_checkpoint_with_validation(&checkpoint_uri, Some(&expected_checksum))
        .await?;
    ...
    let result = store
        .load_checkpoint_with_validation(&checkpoint_uri, Some(wrong_checksum))
        .await;
...
async fn test_tensor_like_data_exchange() -> Result<()> {
    ...
    let loaded_bytes = store
        .get_with_validation(&tensor_uri, Some(&checksum))
        .await?;
    ...
    let partial_bytes = store
        .get_range_with_validation(&tensor_uri, 0, Some(32), None)
        .await?; // First 8 f32s
...
async fn test_distributed_checkpoint_with_validation() -> Result<()> {
    ...
    for rank in 0..world_size {
        ...
        let shard_data = bytes::Bytes::from(format!(
            "rank {} shard data for distributed checkpoint",
            rank
        ));
    ...
    for rank in 0..world_size {
        let shard_data = reader
            .read_shard_by_rank_with_validation(&manifest, rank)
            .await?;
    ...
    let all_shards = reader
        .read_all_shards_concurrent_with_validation(&manifest)
        .await?;
    ...
    assert!(
        is_valid,
        "Distributed checkpoint should validate successfully"
    );
...
async fn test_compression_integration() -> Result<()> {
    ...
    store
        .put(&data_uri, bytes::Bytes::from(large_data.clone()))
        .await?;

    // Test loading with validation (data should compress well)
    let loaded_data = store
        .get_with_validation(&data_uri, Some(&checksum))
        .await?;
    assert_eq!(loaded_data, large_data);

    // Test range loading with validation
    let range_data = store
        .get_range_with_validation(&data_uri, 100, Some(500), None)
        .await?;
...
async fn test_zero_copy_capabilities() -> Result<()> {
    ...
    let loaded_data = store
        .get_with_validation(&zero_copy_uri, Some(&checksum))
        .await?;
...
async fn test_enhanced_error_handling() -> Result<()> {
    ...
    store
        .put(&test_uri, bytes::Bytes::from(test_data.as_ref()))
        .await?;
    ...
    let result = store
        .get_with_validation(&test_uri, Some(wrong_checksum))
        .await;
    assert!(result.is_err(), "Should fail with wrong checksum");
...
async fn test_metadata_preservation() -> Result<()> {
    ...
    let loaded_data = reader
        .read_shard_by_rank_with_validation(&manifest, 0)
        .await?;
