...
fn test_explicit_seed_produces_identical_data() {
    ...
    println!(
        "✓ Generated {} bytes identically with seed {}",
        data1.len(),
        seed
    );
}
...
fn test_seed_vs_system_entropy() {
    ...
    assert_eq!(
        data_seed1, data_seed2,
        "Seeded generator should be deterministic"
    );
    ...
    assert_ne!(
        data_rand1, data_rand2,
        "System entropy generators should be unique"
    );
    println!("✓ System entropy generators are unique");

    // Seeded data should differ from system entropy data
    assert_ne!(
        data_seed1, data_rand1,
        "Seeded should differ from system entropy"
    );
...
fn test_reproducible_across_chunk_sizes() {
    ...
    while let Some(chunk) = obj1.fill_chunk(16 * 1024) {
        // 16 KB chunks
    ...
    while let Some(chunk) = obj2.fill_chunk(256 * 1024) {
        // 256 KB chunks
        data_large_chunks.extend(chunk);
    }

    assert_eq!(
        data_small_chunks.len(),
        data_large_chunks.len(),
        "Sizes should match"
    );
    assert_eq!(
        data_small_chunks, data_large_chunks,
