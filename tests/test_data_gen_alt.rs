...
fn test_generate_controlled_data_alt_basic() -> Result<()> {
    ...
    println!(
        "Note: Minimum size is DGEN_BLOCK_SIZE ({} bytes = 1 MiB)",
        DGEN_BLOCK_SIZE
    );
    ...
    for (size, dedup, compress, description) in test_cases {
        ...
        assert!(
            data.len() >= DGEN_BLOCK_SIZE,
            "Size must be at least DGEN_BLOCK_SIZE for {}",
            description
        );
...
fn test_compressible_with_higher_compress() -> Result<()> {
    ...
    let test_cases = vec![(2, "2x compress"), (3, "3x compress"), (4, "4x compress")];
    ...
    for (compress, description) in test_cases {
        ...
        assert!(
            ratio > 1.1,
            "{} should be compressible, got ratio {:.4}",
            description,
            ratio
        );
...
fn test_deduplication_behavior() -> Result<()> {
    ...
    println!(
        "Note: Algorithm uses DGEN_BLOCK_SIZE ({} bytes = 1 MiB) blocks",
        DGEN_BLOCK_SIZE
    );
    ...
    println!(
        "Expected unique: {} (dedup=2 means 50% unique)",
        num_blocks / 2
    );
    ...
    assert!(
        (actual_unique as i32 - expected_unique as i32).abs() <= 1,
        "Expected ~{} unique blocks, got {}",
        expected_unique,
        actual_unique
    );
...
fn test_streaming_generator() -> Result<()> {
    ...
    assert_eq!(
        streamed_data.len(),
        total_size,
        "Should stream exactly total_size bytes"
    );
