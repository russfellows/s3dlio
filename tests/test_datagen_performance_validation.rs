...
use s3dlio::data_gen_alt::{generate_data_simple, DataGenerator, GeneratorConfig, NumaMode};
...
fn test_datagen_streaming_throughput_100gb() {
    ...
    println!(
        "Warmup: {} GB",
        WARMUP_SIZE as f64 / 1024.0 / 1024.0 / 1024.0
    );
    println!(
        "Test size: {} GB",
        TEST_SIZE as f64 / 1024.0 / 1024.0 / 1024.0
    );
    ...
    println!(
        "  Minimum:   {:.2} GB/s (dgen-rs achieves ~47 GB/s)",
        MIN_THROUGHPUT_GBS
    );
    ...
    println!(
        "✅ PASSED: Throughput {:.2} GB/s meets minimum {:.2} GB/s",
        throughput_gbs, MIN_THROUGHPUT_GBS
    );
}
...
fn test_datagen_api_sanity() {
    ...
    println!(
        "✅ API sanity test passed: generated {} MB",
...
fn test_datagen_batch_api() {
    ...
    assert!(
        non_zero > SIZE / 2,
        "Data should be mostly non-zero (got {} non-zero bytes)",
        non_zero
    );

    println!(
        "✅ Batch API test passed: generated {} MB",
        SIZE / (1024 * 1024)
    );
}
...
fn test_datagen_dedup_compress_ratios() {
    ...
    println!(
        "Dedup=2: {} unique blocks out of {} (expected ~{})",
        actual_unique,
    ...
    // Allow 10% tolerance
    let ratio_diff =
        (actual_unique as f64 - expected_unique as f64).abs() / (expected_unique as f64);
    assert!(
        ratio_diff < 0.1,
        "Dedup ratio off: got {} unique, expected ~{}",
        actual_unique,
        expected_unique
    );
    ...
    println!(
        "Compress=2: {:.1}% zeros (expected ~50%)",
        zero_ratio * 100.0
    );

    assert!(
        (zero_ratio - 0.5).abs() < 0.1,
        "Compression ratio off: got {:.1}% zeros, expected ~50%",
        zero_ratio * 100.0
    );
