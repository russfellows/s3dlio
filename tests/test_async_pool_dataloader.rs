    ...
    for &pool_size in &[2, 8] {
        ...
        while let Some(batch_result) = stream.next().await {
            ...
            assert_eq!(
                batch.len(),
                3,
                "All batches should be size 3 with drop_last=true"
            );
        }

        assert_eq!(
            total_items, 12,
            "Should process 12 items (4 complete batches)"
        );
    }
...
async fn test_error_handling() -> Result<()> {
    ...
    let uris = vec![
        "file:///nonexistent/path/file1.dat".to_string(),
        "file:///nonexistent/path/file2.dat".to_string(),
    ];
    ...
    if let Some(batch_result) = stream.next().await {
        assert!(
            batch_result.is_err(),
            "Should get error for nonexistent files"
        );
    }
