...
use s3dlio::object_store::{CompressionConfig, ObjectWriter};
...
async fn test_compression_basic_functionality() -> Result<()> {
    ...
    assert!(
        compressed_size < test_data.len() as u64,
    ...
    println!(
        "✓ Compression test passed: {} bytes -> {} bytes (ratio: {:.2})",
        test_data.len(),
        compressed_size,
...
async fn test_compression_checksum_integrity() -> Result<()> {
    ...
    assert!(
        checksum.starts_with("crc32c:"),
        "Checksum should have crc32c format"
    );
    assert_eq!(
        checksum.len(),
        15,
        "Checksum should be 'crc32c:' + 8 hex chars"
    );
...
async fn test_compression_disabled() -> Result<()> {
    ...
    println!(
        "✓ No compression test passed: {} bytes uncompressed",
        test_data.len()
    );
