...
async fn test_object_store_integrity_validation() -> Result<()> {
    ...
    let validated_data = store
        .get_with_validation(&test_uri, Some(&expected_checksum))
        .await?;
...
async fn test_range_validation() -> Result<()> {
    ...
    let validated_range = store
        .get_range_with_validation(&test_uri, 5, Some(10), Some(&range_checksum))
        .await?;
    ...
    let result = store
        .get_range_with_validation(&test_uri, 5, Some(10), Some(wrong_checksum))
        .await;
    assert!(result.is_err());
...
async fn test_checkpoint_integrity_validation() -> Result<()> {
    ...
    for (rank, data) in &shard_data {
        ...
        // Write shard data
        store
            .put(&shard_uri, bytes::Bytes::from(data.clone()))
            .await?;
    ...
    store
        .put(&manifest_uri, bytes::Bytes::from(manifest_data))
        .await?;
...
async fn test_corrupted_checkpoint_detection() -> Result<()> {
    ...
    assert!(
        !validation_result,
        "Checkpoint validation should fail for corrupted data"
    );
...
async fn test_checkpoint_loading_with_validation() -> Result<()> {
    ...
    let loaded_data = store
        .load_checkpoint_with_validation(&checkpoint_uri, Some(&checksum))
        .await?;
    ...
    let loaded_no_validation = store
        .load_checkpoint_with_validation(&checkpoint_uri, None)
        .await?;
