...
fn test_streaming_consistency_various_sizes() {
    ...
    for &(size, dedup, compress) in &test_cases {
        ...
        assert_eq!(
            streaming_result, regenerated,
            "Reset consistency failed for size={}, dedup={}, compress={}",
            size, dedup, compress
        );
        ...
        if compress > 1 {
            ...
            assert!(
                zero_count >= expected_zeros - tolerance
                    && zero_count <= expected_zeros + tolerance,
                "Compression ratio incorrect: expected ~{} zeros, got {}",
                expected_zeros,
                zero_count
            );
        }
...
fn test_streaming_different_chunk_sizes() {
    ...
    for &chunk_size in &test_chunk_sizes {
        ...
        assert_eq!(
            result.len(),
            size,
            "Length mismatch for chunk size {}",
            chunk_size
        );
...
fn test_fill_remaining() {
    ...
    assert_eq!(
        combined, full_data,
        "Chunked and full generation should produce identical results"
    );
}
...
fn test_partial_block_at_end() {
    ...
    assert_eq!(
        streaming_data, regenerated_data,
        "Chunked and full generation should be consistent"
    );
}
...
fn test_streaming_performance() {
    ...
    println!(
        ...
        single_pass_duration / iterations
    );
    println!(
        "Streaming: {:?} per iteration",
        streaming_duration / iterations
    );
    ...
    assert!(
        overhead_ratio < 3.0,
        "Streaming overhead too high: {:.2}x slower",
        overhead_ratio
    );
}
