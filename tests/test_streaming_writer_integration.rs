...
use s3dlio::{
    ...
    object_store::{CompressionConfig, ObjectWriter, WriterOptions},
...
async fn test_streaming_data_writer_basic() -> Result<()> {
    ...
    while !writer.is_complete() {
        ...
        println!(
            "Generated {} bytes, total: {}",
            bytes_written, total_generated
        );
    }
...
async fn test_streaming_data_writer_compression() -> Result<()> {
    ...
    assert!(
        ...
        "Compressed file should exist with .zst extension"
    );
...
async fn test_streaming_data_writer_consistency() -> Result<()> {
    ...
    let mut streaming_writer =
        StreamingDataWriter::new(&uri, size, dedup, compress, &store, options.clone()).await?;
    ...
    let mut streaming_writer2 =
        StreamingDataWriter::new(&uri2, size, dedup, compress, &store, options).await?;
    ...
    assert_ne!(
        streaming_checksum, streaming_checksum2,
    ...
    assert_ne!(
        ...
        "Data should be different due to different entropy"
    );
