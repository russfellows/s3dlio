...
use anyhow::Result;
use s3dlio::{infer_scheme, store_for_uri, Scheme};
...
async fn test_file_scheme_inference() {
    ...
    assert_eq!(
        infer_scheme("https://container.blob.core.windows.net/test.txt"),
        Scheme::Azure
    );
    ...
    assert_eq!(
        infer_scheme("https://example.com/test.txt"),
        Scheme::Unknown
    );
...
async fn test_file_store_factory() -> Result<()> {
    ...
    assert!(store
        .exists("file:///this-file-does-not-exist-12345")
        .await
        .is_ok());
...
async fn test_file_store_basic_operations() -> Result<()> {
    ...
    // Test put operation
    store
        .put(&file_uri, bytes::Bytes::from(test_data.as_ref()))
        .await?;
    ...
    store
        .put(&file_uri, bytes::Bytes::from_static(updated_data))
        .await?;
...
async fn test_file_store_directory_operations() -> Result<()> {
    ...
    store
        .put(&file1_uri, bytes::Bytes::from_static(b"File 1 content"))
        .await?;
    store
...
async fn test_file_store_copy_operations() -> Result<()> {
    ...
    store
        .put(&src_uri, bytes::Bytes::from(test_data.as_ref()))
        .await?;
...
async fn test_file_store_error_handling() -> Result<()> {
    ...
    assert!(store
        .stat("file:///this/path/does/not/exist")
        .await
        .is_err());
...
async fn test_file_store_relative_paths() -> Result<()> {
    ...
    store
        .put("file://./test.txt", bytes::Bytes::from(test_data.as_ref()))
        .await?;
