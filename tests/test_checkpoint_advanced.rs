...
async fn test_checkpoint_performance() {
    ...
    for rank in 0..num_ranks {
        ...
        let (layout, shard_meta) = writer
            .save_distributed_shard(1, 1, "pytorch", &test_data)
            .await
            .unwrap();
    ...
    let shards: Vec<_> = layouts_and_shards
        .iter()
        .map(|(_, shard)| shard.clone())
        .collect();
    let layout = &layouts_and_shards[0].0;
    coordinator
        .finalize_distributed_checkpoint(layout, "pytorch", 1, shards, None)
        .await
        .unwrap();

    let write_time = start_time.elapsed();
    println!(
        "✓ Wrote {}MB checkpoint ({} ranks) in {:?}",
        (shard_size * num_ranks as usize) / (1024 * 1024),
        num_ranks,
        write_time
    );
...
async fn test_concurrent_checkpoints() {
    ...
    for i in 0..num_concurrent {
        ...
        let handle = tokio::spawn(async move {
            ...
            let manifest_key = writer
                .save_checkpoint(i + 1, epoch, "pytorch", &test_data, None)
                .await
                .unwrap();
    ...
    for handle in handles {
        ...
        println!(
            "✓ Concurrent checkpoint completed: epoch={}, manifest={}",
            result.0, result.1
        );
        results.push(result);
    }
    ...
    assert!(
        latest.epoch >= 1 && latest.epoch <= num_concurrent,
        "Expected epoch between 1 and {}, got {}",
        num_concurrent,
        latest.epoch
    );
...
async fn test_checkpoint_cleanup() {
    ...
    for (i, epoch) in epochs.iter().enumerate() {
        ...
        writer
            .save_checkpoint(i as u64 + 1, *epoch, "tensorflow", &test_data, None)
            .await
            .unwrap();
    }
...
async fn test_error_recovery() {
    ...
    writer2
        .save_checkpoint(2, 2, "pytorch", b"complete checkpoint data", None)
        .await
        .unwrap();
