...
async fn test_real_world_performance_benchmark() -> Result<()> {
    ...
    for (size_name, size) in &test_cases {
        for (config_name, dedup, compress) in &data_configs {
            let (single_pass_gbps, streaming_gbps) =
                benchmark_real_world_scenario(*size, *dedup, *compress).await?;
...
async fn benchmark_real_world_scenario(
    ...
    for _ in 0..WARMUP_ITERATIONS {
        let _data =
            data_gen_alt::generate_controlled_data_alt(size, dedup, compress, None).to_vec();
...
fn analyze_results(results: &[(usize, usize, usize, f64, f64, &str)]) {
    ...
    let single_pass_wins = results
        .iter()
        .filter(|(_, _, _, _, _, winner)| *winner == "Single-Pass")
        .count();
    let streaming_wins = results
        .iter()
        .filter(|(_, _, _, _, _, winner)| *winner == "Streaming")
        .count();
    ...
    let small_objects: Vec<_> = results
        .iter()
        .filter(|(size, _, _, _, _, _)| *size <= 8 * 1024 * 1024)
        .collect();
    let medium_objects: Vec<_> = results
        .iter()
        .filter(|(size, _, _, _, _, _)| *size > 8 * 1024 * 1024 && *size <= 32 * 1024 * 1024)
        .collect();
    let large_objects: Vec<_> = results
        .iter()
        .filter(|(size, _, _, _, _, _)| *size > 32 * 1024 * 1024)
        .collect();
    ...
    let no_dedup_compress: Vec<_> = results
        .iter()
        .filter(|(_, dedup, compress, _, _, _)| *dedup == 1 && *compress == 1)
        .collect();
    let with_dedup_compress: Vec<_> = results
        .iter()
        .filter(|(_, dedup, compress, _, _, _)| *dedup > 1 || *compress > 1)
        .collect();
    ...
    if single_pass_wins > streaming_wins {
        ...
        println!(
            "   - Single-pass wins in {}/{} scenarios ({:.1}%)",
            single_pass_wins,
            results.len(),
            (single_pass_wins as f64 / results.len() as f64) * 100.0
        );
        ...
        println!(
            "   - Streaming wins in {}/{} scenarios ({:.1}%)",
            streaming_wins,
            results.len(),
            (streaming_wins as f64 / results.len() as f64) * 100.0
        );
...
fn analyze_size_range(name: &str, objects: &[&(usize, usize, usize, f64, f64, &str)]) {
    ...
    let single_pass_wins = objects
        .iter()
        .filter(|(_, _, _, _, _, winner)| *winner == "Single-Pass")
        .count();
    let streaming_wins = objects
        .iter()
        .filter(|(_, _, _, _, _, winner)| *winner == "Streaming")
        .count();

    let avg_single_pass: f64 =
        objects.iter().map(|(_, _, _, sp, _, _)| sp).sum::<f64>() / objects.len() as f64;
    let avg_streaming: f64 =
        objects.iter().map(|(_, _, _, _, st, _)| st).sum::<f64>() / objects.len() as f64;

    let winner = if single_pass_wins > streaming_wins {
        "Single-Pass"
    } else {
        "Streaming"
    };
    ...
    println!(
        "- {}: {} wins ({} vs {}), Avg perf: {:.1} GB/s vs {:.1} GB/s → **Use {}**",
...
async fn test_multiple_8mb_objects_with_dedup_compress() -> Result<()> {
    ...
    for i in 0..NUM_OBJECTS {
        ...
        let data = data_gen_alt::generate_controlled_data_alt(
            OBJECT_SIZE,
            DEDUP_FACTOR,
            COMPRESS_FACTOR,
            None,
        ...
        if i < 3 {
            // Show details for first 3 objects
            println!(
                "  Object {}: {} bytes generated in {:.1}ms",
                i + 1,
                data.len(),
                obj_time.as_millis()
            );
    ...
    println!("|--------|------------|------------|--------|");
    println!(
        "| Single-Pass | {:.1}ms | {:.2} GB/s | {} |",
        single_pass_total_time.as_millis(),
        single_pass_gbps,
    ...
    println!(
        "| Streaming | {:.1}ms | {:.2} GB/s | {} |",
        streaming_total_time.as_millis(),
        streaming_gbps,
        if streaming_gbps > single_pass_gbps {
            "✅"
        } else {
            ""
        }
    ...
    let winner = if single_pass_gbps > streaming_gbps {
        "Single-Pass"
    } else {
        "Streaming"
    };
    ...
    println!(
        "🏆 {} is {:.1}x faster ({:.2} GB/s vs {:.2} GB/s)",
        winner,
        advantage,
        if single_pass_gbps > streaming_gbps {
            single_pass_gbps
    ...
    println!("\n**Memory Usage Analysis**:");
    println!(
        "- Single-Pass peak memory: {:.1}MB per object",
        single_pass_peak_memory as f64 / (1024.0 * 1024.0)
    );
    println!(
        "- Streaming peak memory: {:.1}MB per object",
        streaming_peak_memory as f64 / (1024.0 * 1024.0)
    );
