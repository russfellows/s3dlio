    ...
    for scheme, uri in all_uris.items():
        ...
        try:
            ...
            if any(ok in error_msg for ok in ok_errors):
                print(
                    f"  ✓ {scheme}:// - scheme recognized (I/O error: {type(e).__name__})"
                )
            else:
                assert (
                    "invalid scheme" not in error_msg
                    and "unknown scheme" not in error_msg
                ), f"{scheme}:// - FAILED: scheme not recognized ({e})"
...
def test_local_file_backend():
    ...
    try:
        ...
        print("  ✓ list_full_uris works, scheme preserved")

        # Test stat_object
        stat = s3dlio.stat_object(file_uri)
        assert stat is not None
        print("  ✓ stat_object works")
...
def test_cloud_uri_format_validation():
    ...
    for scheme, uri, expected_backend in test_cases:
        try:
            ...
            if scheme != "s3":
                assert "s3://" not in error_msg.lower(), (
                    f"{scheme}:// - ERROR: S3-specific message for {expected_backend}: {e}"
                )
...
def run_all_tests():
    ...
    for name, test in [
        ("Scheme detection", test_scheme_detection),
        ("Local file backend", test_local_file_backend),
        ("Cloud URI format", test_cloud_uri_format_validation),
        ("Deprecated aliases", test_deprecated_aliases_exist),
    ]:
        try:
            test()
