...
def test_pytorch_integration():
    ...
    try:
        ...
        pytest.skip(reason="PyTorch not available")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        shard_key = writer.save_distributed_shard(
            step=100, epoch=10, framework="pytorch", data=serialized_data
        )
...
def test_jax_integration():
    ...
    try:
        ...
        pytest.skip(reason="JAX not available")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        writer = store.writer(world_size=1, rank=0)
        shard_key = writer.save_distributed_shard(
            step=200, epoch=20, framework="jax", data=serialized_data
        )

        manifest_key = writer.finalize_distributed_checkpoint(
            step=200, epoch=20, framework="jax", shard_metas=[shard_key], user_meta=None
        )
...
def test_tensorflow_integration():
    ...
    try:
        ...
        pytest.skip(reason="TensorFlow not available")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        writer = store.writer(world_size=1, rank=0)
        shard_key = writer.save_distributed_shard(
            step=300, epoch=30, framework="tensorflow", data=serialized_data
        )
        ...
        # Check that arrays match
        assert np.allclose(
            model_vars["dense_weight"].numpy(), loaded_numpy["dense_weight"]
        )
        assert np.allclose(model_vars["dense_bias"].numpy(), loaded_numpy["dense_bias"])
        assert np.allclose(
            model_vars["learning_rate"].numpy(), loaded_numpy["learning_rate"]
        )
...
def test_multi_backend_checkpointing():
    ...
    for strategy in strategies:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            shard_key = writer.save_distributed_shard(
                step=1, epoch=1, framework="test", data=test_data
            )
            ...
            # NOTE: loaded_data is a zero-copy BytesView with no __eq__, so
            # comparing it directly against `bytes` is always False. Wrap in
            # bytes() first (see test_checkpoint_basic_python.py fix).
            assert bytes(loaded_data) == test_data
