    ...
    try:
        ...
        for char in content:
            if char not in " \n0123456789":
                ...
                if not char.isprintable():
                    return (
                        False,
                        f"Index contains non-printable character: {repr(char)}",
                    )
        ...
        for i, line in enumerate(lines):
            ...
            if len(parts) != 2:
                return (
                    False,
                    f"Line {i + 1}: Expected 2 space-separated values, got {len(parts)}",
                )
            ...
            if i > 0:
                ...
                if offset <= prev_offset:
                    return (
                        False,
                        f"Line {i + 1}: Offset {offset} not greater than previous {prev_offset}",
                    )
...
def test_dali_format_compliance():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert is_valid, f"DALI format validation failed: {message}"
        print(f"\n✓ PASS: {message}")
        ...
        with open(tfrecord_path, "rb") as f:
            for i in indices:
                ...
                assert actual_data == expected, (
                    f"Record {i}: MISMATCH, expected {expected!r}, got {actual_data!r}"
                )
...
def test_index_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, (from_bytes, from_file) in enumerate(
            zip(entries_from_bytes, entries_from_file)
        ):
            assert from_bytes == from_file, (
                f"Mismatch at entry {i}: from bytes {from_bytes}, from file {from_file}"
            )
