...
def test_compression_effectiveness():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        compressed_files = list(
            compressed_dir.rglob("rank-*.bin*")
        )  # Include .zst extension

        assert uncompressed_files and compressed_files, (
            "Could not find shard files to compare"
        )
        ...
        assert compressed_size < uncompressed_size, (
            "Compression not effective - files are same size"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n❌ Compression verification failed: {e}")
