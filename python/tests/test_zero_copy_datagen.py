...
def test_basic_generation():
    ...
    assert len(view) == size, f"Expected memoryview of {size} bytes, got {len(view)}"
    ...
    assert len(data_bytes) == size, (
        f"Expected {size} bytes after bytes() conversion, got {len(data_bytes)}"
    )
...
def test_utility_functions():
    ...
    assert total > 0, "Total CPU count should be positive"
    assert default_threads > 0, "Default thread count should be positive"
    assert default_threads <= total, (
        f"Default threads ({default_threads}) should not exceed total CPUs ({total})"
    )
...
def test_dedup_compress():
    ...
    for dedup, compress, desc in configs:
        ...
        view = memoryview(data)
        assert len(view) == size, f"{desc}: expected {size} bytes, got {len(view)}"
