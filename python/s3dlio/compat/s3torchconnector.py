...
class S3IterableDataset(IterableDataset):
    ...
    def _convert_loader_kwargs(self, kwargs):
        ...
        if "reader_constructor" in kwargs:
            warnings.warn(
                "reader_constructor not directly supported - using default reader_mode"
            )
    ...
    @classmethod
    def from_prefix(
        cls, prefix: str, region: Optional[str] = None, **kwargs
    ) -> "S3IterableDataset":
        """
...
class S3MapDataset(Dataset):
    ...
    def __init__(self, uri: str, region: Optional[str] = None, **kwargs):
        """
        Args:
            uri: Full URI to prefix (e.g., "s3://bucket/prefix/")
    ...
    def __getitem__(self, index: int) -> S3Item:
        ...
        if index < 0 or index >= len(self._full_uris):
            raise IndexError(
                f"Index {index} out of range for dataset of size {len(self._full_uris)}"
            )
...
class S3Client:
    ...
    def list_objects(self, bucket, prefix=""):
        ...
        if (
            ...
            or prefix.startswith("az://")
            or prefix.startswith("gs://")
