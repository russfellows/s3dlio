...
fn bench_buffered_random_read(c: &mut Criterion) {
    ...
    let read_sizes = [4 << 10, 64 << 10, 1 << 20]; // 4KB, 64KB, 1MB
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    let bytes_read = unsafe {
                        libc::read(fd, aligned_buf.as_mut_ptr() as *mut libc::c_void, size)
                    };
                ...
                b.iter(|| {
                    ...
                    let handles: Vec<_> = (0..threads)
                        .map(|i| {
                            ...
                            thread::spawn(move || {
                                let path =
                                    test_dir.join(format!("parallel_write_{}_{}.tmp", threads, i));
